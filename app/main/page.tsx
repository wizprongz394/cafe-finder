...
function priceColor(cost:number) { return cost<=400?T.green:cost<=900?T.caramel:T.red; }
function priceLabel(cost:number) { return cost<=300?"₹":cost<=700?"₹₹":cost<=1200?"₹₹₹":"₹₹₹₹"; }
function categoryEmoji(amenity:string) {
  if(amenity==="cafe") return "☕"; if(amenity==="fast_food") return "🍔"; if(amenity==="bar") return "🍺"; return "🍽️";
}
function placeEmoji(place:EnrichedPlace) {
...
function mapsUrl(place:EnrichedPlace) { return `https://www.google.com/maps/search/?api=1&query=${place.lat},${place.lon}`; }
...
function computeScore(p:EnrichedPlace) {
...
function computeUserPain(place:EnrichedPlace,user:GroupUser) {
...
function SplitPlanCard({match,idx,isFirst,users,onPlaceClick,buildFairnessSummary,generateMeetupFlow,buildExplainPrompt}:{
  ...
  const handleExplain=async()=>{
    ...
    } catch { setPlanError("Couldn't reach explanation service. Try again."); }
  ...
  return (
    <div style={{
      ...
      background:isFirst?"linear-gradient(135deg,rgba(192,133,82,0.1) 0%,rgba(94,48,35,0.07) 100%)":"rgba(243,233,220,0.45)",
      border:`1px solid ${isFirst?"rgba(192,133,82,0.4)":"rgba(192,133,82,0.18)"}`,
      boxShadow:isFirst?"0 6px 24px rgba(192,133,82,0.12)":"none",
      ...
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"10px"}}>
        <div style={{display:"flex",alignItems:"center",gap:"6px"}}>
          ...
          <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"13px",fontWeight:400,letterSpacing:"0.06em",color:isFirst?T.caramel:T.coffee,fontStyle:isFirst?"italic":"normal"}}>
            {isFirst?"Smart Plan":"Alternative"}
          </span>
      ...
      <div style={{display:"flex",flexDirection:"column" as const,gap:"5px",marginBottom:"10px"}}>
        {[match.placeA,match.placeB].map((place,pi)=>(
          <button key={pi} onClick={()=>onPlaceClick(place)} style={{
            width:"100%",display:"flex",alignItems:"center",gap:"9px",
            background:"rgba(255,236,209,0.3)",border:"1px solid rgba(192,133,82,0.15)",
            borderRadius:"10px",padding:"7px 10px",cursor:"pointer",textAlign:"left" as const,
          ...
          onMouseLeave={e=>{e.currentTarget.style.background="rgba(255,236,209,0.3)";e.currentTarget.style.borderColor="rgba(192,133,82,0.15)";}}>
            ...
            <a href={mapsUrl(place)} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
              style={{width:"20px",height:"20px",borderRadius:"50%",display:"flex",alignItems:"center",justifyContent:"center",background:"rgba(192,133,82,0.1)",color:T.coffee,textDecoration:"none",flexShrink:0,transition:"all 0.2s"}}
              onMouseEnter={e=>{e.currentTarget.style.background=T.caramel;e.currentTarget.style.color=T.sand;}}
              onMouseLeave={e=>{e.currentTarget.style.background="rgba(192,133,82,0.1)";e.currentTarget.style.color=T.coffee;}}>
      ...
      <div style={{display:"flex",flexWrap:"wrap" as const,gap:"4px",marginBottom:"9px"}}>
        {users.filter(u=>u.location).map((u,ui)=>{
          const dest=match.assignment[u.id]==="B"?match.placeB:match.placeA;
          return(
            <span key={u.id} style={{fontSize:"9px",padding:"3px 8px",borderRadius:"20px",background:"rgba(255,236,209,0.25)",color:T.coffee,borderLeft:`2px solid ${userColors[ui%userColors.length]}`}}>
              {u.label} → {placeEmoji(dest)}
            </span>
          );
        })}
      </div>
...
export default function MainApp() {
  ...
  const fetchMapboxPlaces=useCallback(async(lat:number,lon:number):Promise<RawPlace[]>=>{
    ...
    for(const data of responses) {
      ...
      for(const f of data.features) {
        ...
        results.push({lat:fLat,lon:fLon,tags:{
          name:f.properties?.name??"Unnamed",amenity,
          cuisine:cats.filter(c=>!["food","restaurant","cafe"].includes(c.toLowerCase())).join(", "),
  ...
  const loadPlaces=useCallback(async(centerLat:number,centerLon:number)=>{
    ...
    try {
      ...
      const enriched=Array.from(seen.values()).map(p=>{
        ...
        const base:EnrichedPlace={...p,distance,estimatedCostForTwo:cost,priceSource:source as any,score:0,tags:{...p.tags,name:p.tags?.name||"Unnamed Place"}};
  ...
  useEffect(()=>{
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      ()=>setFetchError("Location access denied. Please search for a location.")
    );
  ...
  function useCurrentLocation(){
    navigator.geolocation.getCurrentPosition(
      pos=>{const loc={latitude:pos.coords.latitude,longitude:pos.coords.longitude};setUserLocation(loc);setSearchLocation(loc);setLocationQuery(`${pos.coords.latitude.toFixed(4)}, ${pos.coords.longitude.toFixed(4)}`);},
  ...
  function placeKey(p:EnrichedPlace){return `${p.tags.name}-${p.lat.toFixed(4)}-${p.lon.toFixed(4)}`;}
  ...
  function handlePlaceClick(place:EnrichedPlace){setSelectedPlace({...place});}

  const selStyle={
    background:"rgba(243,233,220,0.85)",
    border:"1px solid rgba(192,133,82,0.28)",
    borderRadius:"9px",padding:"0 26px 0 10px",
    fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:300 as const,
    color:T.chocolate,outline:"none",cursor:"pointer",
    ...
    <div style={{height:"100vh",background:T.cream,display:"flex",flexDirection:"column" as const,overflow:"hidden",fontFamily:"'DM Sans',sans-serif"}}>
      ...
      <style>{`
        ...
        *{box-sizing:border-box;margin:0;padding:0;}
        ...
        .xscroll::-webkit-scrollbar-track{background:transparent;}
        .xscroll::-webkit-scrollbar-thumb{background:rgba(192,133,82,0.35);border-radius:4px;}
        .xscroll::-webkit-scrollbar-thumb:hover{background:#C08552;}
        ...
        @keyframes dotDance{0%,100%{transform:translateY(0);opacity:0.3}40%{transform:translateY(-4px);opacity:0.9}}
        @keyframes brewPulse{0%,100%{opacity:0.4;transform:scale(0.97)}50%{opacity:1;transform:scale(1)}}

        /* ── MAP MARKER STYLES ──────────────────────────────────── */
        /* Pop animation for every new marker rendered by MapView */
        @keyframes markerPop{
          0%{opacity:0;transform:scale(0.4) translateY(8px)}
          65%{opacity:1;transform:scale(1.12) translateY(-2px)}
          100%{opacity:1;transform:scale(1) translateY(0)}
        }
        /* Ripple pulse for user-location marker */
        @keyframes markerRipple{
          0%{box-shadow:0 0 0 0 rgba(192,133,82,0.5),0 3px 10px rgba(62,0,12,0.25)}
          70%{box-shadow:0 0 0 10px rgba(192,133,82,0),0 3px 10px rgba(62,0,12,0.25)}
          100%{box-shadow:0 0 0 0 rgba(192,133,82,0),0 3px 10px rgba(62,0,12,0.25)}
        }

        /* Mapbox GL markers - smooth pop and hover */
        .mapboxgl-marker{
          animation:markerPop 0.4s cubic-bezier(0.22,1,0.36,1) both !important;
        ...
        /* Custom EXPRESSO pin marker */
        .exp-pin{
          width:34px;height:44px;position:relative;cursor:pointer;
          animation:markerPop 0.38s cubic-bezier(0.22,1,0.36,1) both;
          filter:drop-shadow(0 3px 7px rgba(62,0,12,0.28));
          transition:transform 0.2s cubic-bezier(0.22,1,0.36,1),filter 0.2s;
        }
        .exp-pin:hover{
          transform:scale(1.18) translateY(-3px);
          filter:drop-shadow(0 5px 12px rgba(62,0,12,0.35));
        }
        .exp-pin.exp-pin--selected{
          transform:scale(1.22) translateY(-4px);
          filter:drop-shadow(0 6px 16px rgba(192,133,82,0.55));
        }
        /* SVG inside pin */
        .exp-pin .pin-body{transition:fill 0.2s;}
        .exp-pin:hover .pin-body,.exp-pin.exp-pin--selected .pin-body{fill:#C08552 !important;}
        /* Emoji inside pin */
        .exp-pin .pin-icon{
        ...
        .exp-user-dot{
          ...
          background:#C08552;border:3px solid #FFECD1;
        ...
        .exp-group-marker{
          ...
          background:#3E000C;border:2px solid #FFECD1;
          display:flex;align-items:center;justify-content:center;
          font-family:'DM Sans',sans-serif;font-size:9px;font-weight:500;
          color:#FFECD1;letter-spacing:0.03em;white-space:nowrap;
          box-shadow:0 2px 8px rgba(62,0,12,0.3);
          animation:markerPop 0.38s cubic-bezier(0.22,1,0.36,1) both;
          cursor:default;
        }

        /* Place card accent bar */
        ...
        .place-card:hover::before{background:rgba(192,133,82,0.45);}
        .place-card.sel::before{background:#C08552;}

        /* Focus rings */
        button:focus-visible,select:focus-visible,input:focus-visible,a:focus-visible{
          outline:2px solid #C08552;outline-offset:2px;
        }
      `}</style>
      ...
      <header style={{
        ...
        background:"rgba(255,236,209,0.97)",
        backdropFilter:"blur(16px)",
        borderBottom:"1px solid rgba(192,133,82,0.2)",
        boxShadow:"0 2px 16px rgba(62,0,12,0.06)",
        position:"relative",zIndex:30,
        animation:mounted?"headerIn 0.55s cubic-bezier(0.22,1,0.36,1) both":"none",
      }}>
        ...
        <div style={{maxWidth:"1400px",margin:"0 auto",display:"flex",alignItems:"center",gap:"14px",flexWrap:"wrap" as const,rowGap:"8px"}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:"9px",flexShrink:0}}>
            <div style={{position:"relative",width:"28px",height:"28px",display:"flex",alignItems:"center",justifyContent:"center"}}>
              ...
              <span style={{fontSize:"15px",position:"relative"}}>☕</span>
            </div>
            <span style={{fontFamily:"'Cormorant Garamond',serif",fontWeight:300,fontStyle:"italic",fontSize:"21px",letterSpacing:"0.1em",color:T.chocolate,lineHeight:"1"}}>
              Expr<span style={{color:T.caramel,fontStyle:"normal"}}>e</span>sso
            </span>
          </div>

          <div style={{flex:1,position:"relative",display:"flex",gap:"8px",alignItems:"center",minWidth:"180px"}}>
            <div style={{position:"relative",flex:1}}>
              ...
              <input ref={inputRef} type="text" placeholder="Search neighbourhood, city, or address…"
                value={locationQuery}
                onChange={e=>{setLocationQuery(e.target.value);fetchSuggestions(e.target.value);}}
                onKeyDown={handleKeyDown}
                style={{
                  width:"100%",height:"34px",background:"rgba(243,233,220,0.8)",
                  border:"1px solid rgba(192,133,82,0.25)",borderRadius:"10px",
                  padding:"0 14px 0 34px",fontFamily:"'DM Sans',sans-serif",
                  fontSize:"12px",fontWeight:300,color:T.chocolate,outline:"none",
              ...
              {suggestions.length>0&&(
                <div ref={suggestionsRef} style={{
                  ...
                  background:T.sand,border:"1px solid rgba(192,133,82,0.25)",borderRadius:"12px",
                  overflow:"hidden",boxShadow:"0 12px 40px rgba(62,0,12,0.13)",
                  ...
                  {suggestions.map((s,i)=>(
                    <button key={s.place_name} onMouseDown={e=>{e.preventDefault();selectSuggestion(s);}}
                      style={{
                        ...
                        background:i===activeIndex?"rgba(192,133,82,0.1)":"none",
                        border:"none",borderBottom:i<suggestions.length-1?"1px solid rgba(192,133,82,0.07)":"none",
                        fontFamily:"'DM Sans',sans-serif",fontSize:"12px",fontWeight:300,
                        color:i===activeIndex?T.chocolate:T.coffee,cursor:"pointer",
                        transition:"background 0.12s,color 0.12s",display:"block",
                      }}
                      onMouseEnter={e=>{e.currentTarget.style.background="rgba(192,133,82,0.08)";e.currentTarget.style.color=T.chocolate;}}
                      onMouseLeave={e=>{e.currentTarget.style.background=i===activeIndex?"rgba(192,133,82,0.1)":"none";e.currentTarget.style.color=i===activeIndex?T.chocolate:T.coffee;}}
                    >{s.place_name}</button>
                  ))}
                </div>
              )}
            ...
            <button onClick={handleSearch} disabled={searching} style={{
              height:"34px",padding:"0 16px",background:T.chocolate,color:T.sand,
              border:"none",borderRadius:"10px",fontFamily:"'DM Sans',sans-serif",
              fontSize:"12px",fontWeight:400,letterSpacing:"0.08em",cursor:"pointer",
              transition:"background 0.2s,transform 0.15s",whiteSpace:"nowrap" as const,
            }}
            onMouseEnter={e=>{if(!searching)e.currentTarget.style.background=T.brownie;}}
            onMouseLeave={e=>{e.currentTarget.style.background=T.chocolate;}}
            onMouseDown={e=>{e.currentTarget.style.transform="scale(0.97)";}}
            onMouseUp={e=>{e.currentTarget.style.transform="scale(1)";}}>
              {searching?"…":"Search"}
            </button>
            <button onClick={useCurrentLocation} title="Use my location" style={{
              width:"34px",height:"34px",flexShrink:0,
              background:"rgba(243,233,220,0.8)",border:"1px solid rgba(192,133,82,0.25)",
              borderRadius:"10px",display:"flex",alignItems:"center",justifyContent:"center",
              fontSize:"14px",cursor:"pointer",transition:"all 0.18s",
            }}
            onMouseEnter={e=>{e.currentTarget.style.background=T.sand;e.currentTarget.style.borderColor="rgba(192,133,82,0.5)";}}
            onMouseLeave={e=>{e.currentTarget.style.background="rgba(243,233,220,0.8)";e.currentTarget.style.borderColor="rgba(192,133,82,0.25)";}}>
              🧭
            </button>
          </div>

          <div style={{display:"flex",alignItems:"center",gap:"8px",flexShrink:0,flexWrap:"wrap" as const,rowGap:"6px"}}>
            <button onClick={()=>{setGroupMode(!groupMode);if(!groupMode)setShowGroupPanel(true);}} style={{
              height:"34px",padding:"0 12px",
              background:groupMode?"rgba(192,133,82,0.12)":"none",
              border:`1px solid ${groupMode?T.caramel:"rgba(192,133,82,0.28)"}`,
              borderRadius:"9px",fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:400,
              letterSpacing:"0.06em",color:groupMode?T.brownie:T.coffee,cursor:"pointer",
              transition:"all 0.2s",whiteSpace:"nowrap" as const,
            }}
            onMouseEnter={e=>{e.currentTarget.style.background="rgba(192,133,82,0.1)";e.currentTarget.style.borderColor=T.caramel;e.currentTarget.style.color=T.brownie;}}
            onMouseLeave={e=>{e.currentTarget.style.background=groupMode?"rgba(192,133,82,0.12)":"none";e.currentTarget.style.borderColor=groupMode?T.caramel:"rgba(192,133,82,0.28)";e.currentTarget.style.color=groupMode?T.brownie:T.coffee;}}>
              {groupMode?"👥 Group":"👤 Solo"}
            </button>

            <button onClick={handleSignOut} style={{
              height:"34px",padding:"0 12px",background:"none",
              border:"1px solid rgba(158,58,58,0.22)",borderRadius:"9px",
              fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:400,
              letterSpacing:"0.06em",color:"#9e3a3a",cursor:"pointer",
              transition:"all 0.2s",whiteSpace:"nowrap" as const,
            }}
            onMouseEnter={e=>{e.currentTarget.style.background="rgba(158,58,58,0.07)";e.currentTarget.style.borderColor="rgba(158,58,58,0.45)";}}
            onMouseLeave={e=>{e.currentTarget.style.background="none";e.currentTarget.style.borderColor="rgba(158,58,58,0.22)";}}>
              🚪 Sign out
            </button>
      ...
      <div style={{flex:1,display:"flex",overflow:"hidden",padding:"14px 20px",gap:"12px",maxWidth:"1400px",width:"100%",margin:"0 auto",minHeight:0}}>
        ...
        <div style={{
          ...
          boxShadow:"0 4px 28px rgba(62,0,12,0.09)",
          position:"relative",
          ...
          {mapCenter?(
            ...
            <div style={{height:"100%",background:"rgba(243,233,220,0.4)",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <div style={{width:"70%",height:"50%",background:"linear-gradient(90deg,rgba(192,133,82,0.07) 25%,rgba(192,133,82,0.14) 50%,rgba(192,133,82,0.07) 75%)",backgroundSize:"200% 100%",borderRadius:"16px",animation:"shimmer 1.4s ease-in-out infinite"}}/>
            </div>
          ):(
            <div style={{height:"100%",display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",background:"rgba(243,233,220,0.45)",gap:"12px"}}>
              <span style={{fontSize:"36px",opacity:0.4}}>🗺️</span>
              <span style={{fontFamily:"'Cormorant Garamond',serif",fontSize:"18px",fontWeight:300,color:T.coffee,letterSpacing:"0.06em"}}>Waiting for location…</span>
              <span style={{fontSize:"11px",color:"rgba(137,87,55,0.45)",fontWeight:300,letterSpacing:"0.04em"}}>Allow location access or search above</span>
            </div>
          )}
        ...
        <div style={{display:"flex",gap:"10px",minWidth:0,height:"100%",overflow:"hidden"}}>
          ...
          {groupMode&&!showGroupPanel&&(
            <button onClick={()=>setShowGroupPanel(true)} style={{
              width:"36px",flexShrink:0,alignSelf:"flex-start",
              background:"rgba(192,133,82,0.08)",border:"1px solid rgba(192,133,82,0.25)",
              borderRadius:"12px",height:"72px",
              display:"flex",flexDirection:"column" as const,alignItems:"center",justifyContent:"center",gap:"5px",
              cursor:"pointer",transition:"all 0.2s",position:"relative" as const,
              animation:"panelIn 0.25s ease-out",
            }}
            onMouseEnter={e=>{e.currentTarget.style.background="rgba(192,133,82,0.14)";e.currentTarget.style.borderColor=T.caramel;}}
            onMouseLeave={e=>{e.currentTarget.style.background="rgba(192,133,82,0.08)";e.currentTarget.style.borderColor="rgba(192,133,82,0.25)";}}>
          ...
          {groupMode&&showGroupPanel&&(
            <div style={{
              width:"230px",flexShrink:0,height:"100%",
              background:T.sand,border:"1px solid rgba(192,133,82,0.2)",
              borderRadius:"16px",boxShadow:"0 4px 28px rgba(62,0,12,0.09)",
              display:"flex",flexDirection:"column" as const,overflow:"hidden",
              animation:"groupIn 0.28s cubic-bezier(0.22,1,0.36,1) both",
            }}>
                  ...
                  <div key={user.id} style={{background:"rgba(255,255,255,0.5)",border:"1px solid rgba(192,133,82,0.15)",borderRadius:"12px",padding:"10px 12px",animation:"slideUp 0.22s ease-out both",animationDelay:`${ui*35}ms`,flexShrink:0}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:"8px"}}>
                      <input value={user.label} onChange={e=>updateUser(user.id,{label:e.target.value})}
                        style={{background:"transparent",border:"none",borderBottom:"1px solid rgba(192,133,82,0.25)",fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:500,color:T.chocolate,width:"90px",outline:"none",padding:"1px 0",transition:"border-color 0.2s"}}
                        onFocus={e=>{e.currentTarget.style.borderBottomColor=T.caramel;}}
                        onBlur={e=>{e.currentTarget.style.borderBottomColor="rgba(192,133,82,0.25)";}}>
                      </input>
                    ...
                    <div style={{position:"relative",marginBottom:"6px"}}>
                      <input type="text" placeholder={user.id==="you"?"Your location…":"Their location…"}
                        ...
                        style={{width:"100%",background:"rgba(243,233,220,0.7)",border:"1px solid rgba(192,133,82,0.2)",borderRadius:"7px",padding:"5px 22px 5px 8px",fontFamily:"'DM Sans',sans-serif",fontSize:"10px",fontWeight:300,color:T.chocolate,outline:"none",transition:"all 0.18s"}}
                        onFocus={e=>{e.currentTarget.style.borderColor=T.caramel;e.currentTarget.style.background=T.sand;}}
                        onBlur={e=>{e.currentTarget.style.borderColor="rgba(192,133,82,0.2)";e.currentTarget.style.background="rgba(243,233,220,0.7)";}}/>
                        ...
                        <div style={{position:"absolute",top:"calc(100% + 2px)",left:0,right:0,zIndex:50,background:T.sand,border:"1px solid rgba(192,133,82,0.2)",borderRadius:"8px",overflow:"hidden",boxShadow:"0 4px 16px rgba(62,0,12,0.1)",maxHeight:"108px",overflowY:"auto"}}>
                          {userGeoSugg[user.id].map(s=>(
                            <button key={s.place_name} onMouseDown={e=>{e.preventDefault();selectUserLocation(user.id,s);}}
                              style={{width:"100%",textAlign:"left" as const,padding:"6px 10px",background:"none",border:"none",fontFamily:"'DM Sans',sans-serif",fontSize:"10px",color:T.coffee,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" as const,display:"block",transition:"background 0.1s"}}
                              onMouseEnter={e=>{e.currentTarget.style.background="rgba(192,133,82,0.08)";}}
                              onMouseLeave={e=>{e.currentTarget.style.background="none";}}>{s.place_name}</button>
                          ))}
                        </div>
                      )}
                    </div>
                    ...
                    <select value={user.maxBudget} onChange={e=>updateUser(user.id,{maxBudget:Number(e.target.value)})}
                      style={{width:"100%",background:"rgba(243,233,220,0.7)",border:"1px solid rgba(192,133,82,0.2)",borderRadius:"7px",padding:"4px 8px",fontFamily:"'DM Sans',sans-serif",fontSize:"10px",color:T.chocolate,outline:"none",cursor:"pointer",marginBottom:"6px"}}
                      onFocus={e=>{e.currentTarget.style.borderColor=T.caramel;}}>
                      ...
                      <div style={{display:"flex",flexWrap:"wrap" as const,gap:"3px",marginBottom:"6px"}}>
                        {user.preferences.map(p=>(
                          <span key={p} style={{display:"inline-flex",alignItems:"center",gap:"3px",background:"rgba(192,133,82,0.1)",color:T.brownie,fontSize:"9px",padding:"2px 7px",borderRadius:"20px"}}>
                            {p}
                            <button onClick={()=>removePref(user.id,p)} style={{background:"none",border:"none",color:"rgba(137,87,55,0.45)",cursor:"pointer",fontSize:"10px",lineHeight:"1",padding:0,transition:"color 0.15s"}} onMouseEnter={e=>{e.currentTarget.style.color=T.caramel;}} onMouseLeave={e=>{e.currentTarget.style.color="rgba(137,87,55,0.45)";}}>×</button>
                          </span>
                        ))}
                      </div>
                    )}

                    <input type="text" placeholder="Add preference…" value={prefInput[user.id]??""}
                      onChange={e=>setPrefInput(p=>({...p,[user.id]:e.target.value}))}
                      onKeyDown={e=>{if(e.key==="Enter")addPref(user.id,prefInput[user.id]??"");}}
                      style={{width:"100%",background:"rgba(243,233,220,0.7)",border:"1px solid rgba(192,133,82,0.2)",borderRadius:"7px",padding:"4px 8px",fontFamily:"'DM Sans',sans-serif",fontSize:"10px",fontWeight:300,color:T.chocolate,outline:"none",marginBottom:"5px",transition:"all 0.18s"}}
                      onFocus={e=>{e.currentTarget.style.borderColor=T.caramel;e.currentTarget.style.background=T.sand;}}
                      onBlur={e=>{e.currentTarget.style.borderColor="rgba(192,133,82,0.2)";e.currentTarget.style.background="rgba(243,233,220,0.7)";}}/>
          ...
          <div style={{width:"300px",display:"flex",flexDirection:"column" as const,gap:"8px",height:"100%",overflow:"hidden",animation:"panelIn 0.35s cubic-bezier(0.22,1,0.36,1) 0.12s both"}}>
              ...
              <div style={{flex:1,background:"rgba(243,233,220,0.7)",border:"1px solid rgba(192,133,82,0.2)",borderRadius:"10px",padding:"3px",display:"flex",gap:"2px"}}>
                {(["all","shortlist"] as const).map(tab=>(
                  <button key={tab} onClick={()=>setActiveTab(tab)} style={{
                    ...
                    background:activeTab===tab?T.chocolate:"none",
                    fontFamily:"'DM Sans',sans-serif",fontSize:"11px",fontWeight:400,
                    letterSpacing:"0.04em",color:activeTab===tab?T.sand:"rgba(137,87,55,0.65)",
                    cursor:"pointer",transition:"all 0.22s cubic-bezier(0.22,1,0.36,1)",
                    whiteSpace:"nowrap" as const,
                    boxShadow:activeTab===tab?"0 2px 8px rgba(62,0,12,0.18)":"none",
                  }}
                  onMouseEnter={e=>{if(activeTab!==tab){e.currentTarget.style.background="rgba(192,133,82,0.08)";e.currentTarget.style.color=T.chocolate;}}}
                  onMouseLeave={e=>{if(activeTab!==tab){e.currentTarget.style.background="none";e.currentTarget.style.color="rgba(137,87,55,0.65)";}}}
                  >
                    {tab==="all"?`✦ All${!loading?` (${displayedItems.length})`:""}`:
                    `⭐ Saved (${shortlist.length})`}
                  </button>
                ))}
              </div>
              {cacheStatus&&(
                <span style={{fontSize:"9px",padding:"3px 7px",borderRadius:"6px",flexShrink:0,background:cacheStatus==="LOCAL"?"rgba(74,124,89,0.12)":"rgba(192,133,82,0.1)",color:cacheStatus==="LOCAL"?T.green:T.coffee,border:`1px solid ${cacheStatus==="LOCAL"?"rgba(74,124,89,0.2)":"rgba(192,133,82,0.2)"}`,letterSpacing:"0.04em"}}>
                  {cacheStatus==="LOCAL"?"💾":"🌐"}
                </span>
              )}
            </div>
            ...
            {activeTab==="all"&&!groupMode&&(
              <input type="text" placeholder="🔍 Filter by name or cuisine…" value={resultsQuery} onChange={e=>setResultsQuery(e.target.value)}
                style={{width:"100%",background:"rgba(255,255,255,0.45)",border:"1px solid rgba(192,133,82,0.2)",borderRadius:"9px",padding:"7px 12px",fontFamily:"'DM Sans',sans-serif",fontSize:"12px",fontWeight:300,color:T.chocolate,outline:"none",transition:"all 0.2s",animation:"slideUp 0.28s ease-out",flexShrink:0}}
                onFocus={e=>{e.currentTarget.style.borderColor=T.caramel;e.currentTarget.style.background="rgba(255,236,209,0.5)";e.currentTarget.style.boxShadow="0 0 0 3px rgba(192,133,82,0.08)";}}
                onBlur={e=>{e.currentTarget.style.borderColor="rgba(192,133,82,0.2)";e.currentTarget.style.background="rgba(255,255,255,0.45)";e.currentTarget.style.boxShadow="none";}}/>
            )}

            {groupMode&&activeTab==="all"&&(
              <div style={{fontSize:"10px",fontWeight:300,borderRadius:"9px",padding:"7px 12px",letterSpacing:"0.03em",lineHeight:"1.4",background:shouldUseSplit?"rgba(192,133,82,0.1)":"rgba(243,233,220,0.7)",border:`1px solid ${shouldUseSplit?"rgba(192,133,82,0.3)":"rgba(192,133,82,0.18)"}`,color:shouldUseSplit?T.brownie:T.coffee,animation:"slideUp 0.24s ease-out",flexShrink:0}}>
              ...
              {loading?(
                ...
                <div style={{textAlign:"center",padding:"34px 18px",background:"rgba(255,236,209,0.3)",border:"1px dashed rgba(192,133,82,0.18)",borderRadius:"16px",animation:"fadeIn 0.3s ease-out",flexShrink:0}}>
                  ...
                  <div style={{fontSize:"11px",fontWeight:300,color:"rgba(137,87,55,0.58)",lineHeight:"1.65",letterSpacing:"0.02em"}}>
                    {activeTab==="shortlist"?"Tap ☆ on any place to save it for later.":
                    groupMode?"Add locations and preferences for each person.":
                    resultsQuery?`Nothing matched "${resultsQuery}". Try a broader search.`:
                ...
                (displayedItems as any[]).map((item,idx)=>{
                  if(item.placeA){
                    return(
                      <SplitPlanCard key={`split-${idx}`} match={item} idx={idx} isFirst={idx===0}
                        users={users} onPlaceClick={handlePlaceClick}
                        buildFairnessSummary={()=>"Balanced for everyone"}
                  ...
                  return(
                    <div key={placeKey(place)} onClick={()=>handlePlaceClick(place)}
                      className={`place-card${isSel?" sel":""}`}
                      style={{
                        background:"rgba(255,255,255,0.55)",
                        border:`1px solid ${isSel?T.caramel:"rgba(192,133,82,0.18)"}`,
                        borderRadius:"14px",padding:"11px 13px",
                        cursor:"pointer",overflow:"hidden",
                        transition:"background 0.2s,border-color 0.22s,box-shadow 0.22s,transform 0.2s cubic-bezier(0.22,1,0.36,1)",
                        userSelect:"none" as const,
                        animation:"slideUp 0.3s cubic-bezier(0.22,1,0.36,1) both",
                        animationDelay:`${Math.min(idx*25,280)}ms`,
                        boxShadow:isSel?"0 4px 18px rgba(192,133,82,0.15)":"none",
                        flexShrink:0,
                      }}
                      onMouseEnter={e=>{
                        e.currentTarget.style.background="rgba(255,236,209,0.65)";
                        e.currentTarget.style.borderColor=isSel?T.caramel:"rgba(192,133,82,0.35)";
                        e.currentTarget.style.transform="translateY(-2px) scale(1.005)";
                        e.currentTarget.style.boxShadow="0 6px 20px rgba(62,0,12,0.09)";
                      }}
                      onMouseLeave={e=>{
                        e.currentTarget.style.background="rgba(255,255,255,0.55)";
                        e.currentTarget.style.borderColor=isSel?T.caramel:"rgba(192,133,82,0.18)";
                        e.currentTarget.style.transform="translateY(0) scale(1)";
                        e.currentTarget.style.boxShadow=isSel?"0 4px 18px rgba(192,133,82,0.15)":"none";
                      }}>
                      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",gap:"8px"}}>
                        <div style={{flex:1,minWidth:0}}>
                          <div style={{display:"flex",alignItems:"center",gap:"7px",marginBottom:"2px"}}>
                            <span style={{fontSize:"15px",flexShrink:0}}>{placeEmoji(place)}</span>
                            <span style={{fontFamily:"'DM Sans',sans-serif",fontSize:"12px",fontWeight:500,color:T.chocolate,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"normal",wordBreak:"break-word" as const,lineHeight:"1.3"}}>
                              {place.tags.name}
                            </span>
                          </div>
                        ...
                        <button onClick={e=>{e.stopPropagation();toggleShortlist(place);}} style={{
                          background:"none",border:"none",fontSize:"17px",cursor:"pointer",
                          transition:"transform 0.15s,color 0.15s",padding:"3px",margin:"-3px -3px 0 0",
                          lineHeight:"1",flexShrink:0,color:added?T.caramel:"rgba(137,87,55,0.28)",
                        }}
                        onMouseEnter={e=>{e.currentTarget.style.color=added?T.caramel:"rgba(192,133,82,0.6)";}}
                        onMouseLeave={e=>{e.currentTarget.style.color=added?T.caramel:"rgba(137,87,55,0.28)";}}
                      ...
                      <div style={{display:"flex",justifyContent:"flex-end",marginTop:"5px"}}>
                        <a href={mapsUrl(place)} target="_blank" rel="noopener noreferrer" onClick={e=>e.stopPropagation()}
                          style={{display:"inline-flex",alignItems:"center",gap:"3px",fontSize:"9px",color:"rgba(192,133,82,0.4)",textDecoration:"none",transition:"color 0.18s",letterSpacing:"0.06em"}}
                          onMouseEnter={e=>{e.currentTarget.style.color=T.caramel;}}
                          onMouseLeave={e=>{e.currentTarget.style.color="rgba(192,133,82,0.4)";}}>
